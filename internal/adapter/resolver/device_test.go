...
func TestDeviceResolver_Invalidate(t *testing.T) {
	tests := []struct {
		name    string
	...
	}{
		{
			...
			wantErr: false,
			verify: func(s *miniredis.Miniredis) {
				assert.False(t, s.Exists("device:device-uid-1:id"))
				assert.False(t, s.Exists("device:id:100:uid"))
			},
		},
		{
		...
		},
		{
			...
			verify: func(s *miniredis.Miniredis) {
				assert.False(t, s.Exists("device:device-uid-1:id"))
				assert.False(t, s.Exists("device:id:100:uid"))
				assert.False(t, s.Exists("device:device-uid-2:id"))
				assert.False(t, s.Exists("device:id:200:uid"))
			},
		},
		{
		...
		},
		{
			...
			verify: func(s *miniredis.Miniredis) {
				...
				assert.True(t, s.Exists("device:id:100:uid"))
			},
		},
	}
